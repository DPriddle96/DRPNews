 ...
 */
class NetworkStatusChecker(private val connectivityManager: ConnectivityManager?) : Logger {

    fun hasInternetConnection(): Boolean {
        val network = connectivityManager?.activeNetwork ?: return false
        val capabilities = connectivityManager.getNetworkCapabilities(network) ?: return false
        logDebug("hasInternetConnection - Capabilities: $capabilities")
    ...
    fun hasWifiConnection(): Boolean {
        val network = connectivityManager?.activeNetwork ?: return false
        val capabilities = connectivityManager.getNetworkCapabilities(network) ?: return false
        logDebug("hasWifiConnection - Capabilities: $capabilities")
        return capabilities.hasTransport(NetworkCapabilities.TRANSPORT_WIFI)
    }
